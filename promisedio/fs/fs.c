...
readall_read_callback(uv_fs_t *req)
{
    ACQUIRE_GIL
        ...
        if (req->result == 0) {
            if (PyBytes_GET_SIZE(readstate->buffer) > readstate->nbytes) {
                PyTrack_MarkBeginResize(readstate->buffer);
                if (_PyBytes_Resize(&readstate->buffer, readstate->nbytes) < 0) {
                    Promise_Reject(p, NULL);
                    goto finally;
                }
        ...
        readstate->nbytes += req->result;
        if (readstate->nbytes >= PyBytes_GET_SIZE(readstate->buffer)) {
            size_t new_size = new_buffersize(readstate->nbytes);
        ...
        char *buf = PyBytes_AS_STRING(readstate->buffer) + readstate->nbytes;
        Py_ssize_t size = PyBytes_GET_SIZE(readstate->buffer) - readstate->nbytes;
...
Fs_Readall(_ctx_var, int fd)
{
    ...
    Request_SETUP(SeekReq, req, promise)
    if (req) {
        *Promise_DATA(promise, readallstate) = (readallstate) {.fd = fd, .nbytes = 0};
